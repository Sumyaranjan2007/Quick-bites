/**
 * Taking a photo of a dish and getting it to the server small enough to arrive.
 *
...
async function ensurePermission(source: PhotoSource): Promise<boolean> {
  ...
  Alert.alert(
    ...
    source === 'camera'
      ? 'Turn on camera access in Settings to photograph a dish.'
      : 'Turn on photo access in Settings to choose a dish picture.'
  );
  return false;
}
